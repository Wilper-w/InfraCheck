...
export default function CheckItemsTab() {
  const crud = useCrudTable<CheckItem, CheckItemInput>({
    api: checkItemApi,
    labels: { created: '巡检项已创建', updated: '巡检项已更新', deleted: '巡检项已删除' },
    toFormValues: (r) => ({
      name: r.name,
    ...
    },
    {
      ...
      render: (_: unknown, record: CheckItem) => (
        <Space>
          ...
          <Popconfirm title="确认删除该巡检项？" onOk={() => crud.remove(record.id)}>
  ...
  return (
    <div>
      ...
      <Modal
        title={crud.editing ? '编辑巡检项' : '新建巡检项'}
